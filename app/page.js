        ...
        <div className="nav-links hide-mobile">
          ...
          {session ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginLeft: '1rem' }}>
              ...
              <button onClick={() => signOut()} className="nav-link" style={{ background: 'none', border: 'none', padding: 0 }}>Sign Out</button>
            </div>
          ) : (
            <button onClick={() => signIn('google', { callbackUrl: '/' })} className="btn-primary" style={{ padding: '8px 20px', fontSize: '0.9rem' }}>Sign In</button>
          )}
        </div>
